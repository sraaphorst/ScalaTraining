...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    for ((b2,m) <- b.legalNeighbors.toStream) yield (b2, m :: history)
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, m) #:: xs => {
      ...
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(b, m), newExplored)
      newNeighbors ++ from(xs ++ newNeighbors, newExplored)
    }
  }
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.headOption.map(_._2.reverse).getOrElse(Nil)
}
